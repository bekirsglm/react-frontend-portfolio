...
export default function Home() {
  const sections = [
    ...
    },
    {
      ...
      to: "skills",
      icon: <FaReact size={40} className=" hover:bg-white/10 text-blue-400 rounded-full" />,
    },
    {
      title: "Projelerim",
      description: "Tamamladığım projelerin birkaçı ve kullandığım teknolojiler.",
      to: "projects",
      icon: <FaGithub size={40} className="hover:bg-blue-500 rounded-full text-white" />,
    },
    {
      title: "İletişim",
      description: "Bana ulaşabileceğiniz iletişim kanalları.",
      to: "contact",
      icon: <FaPhone size={40} className="hover:bg-blue-500 rounded-full bg-white/10" />,
    },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-7xl mx-auto">
        {sections.map((section, idx) => (
          <Link
            key={idx}
            to={section.to}
            className="group relative flex flex-col justify-between p-8 rounded-3xl overflow-hidden shadow-xl border border-white/10 transition-transform transform hover:scale-105 bg-white/5 backdrop-blur-md opacity-0 animate-about-fade"
          ...
          >
            ...
            <div className="mt-6 flex items-center gap-3 text-white font-semibold group-hover:text-white/90">
